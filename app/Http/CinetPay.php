  ...
  class CinetPay
  {
     ...
     */
      ...
      if (!empty($param['metadata']))  $this->metadata = $param['metadata'];
        ...
        if ( $flux_json == false)
        dd("Un probleme est survenu lors de l'appel du WS !");
      ...
      public function CheckDataExist($param, $action)// a customiser pour la function check status
      {
        ...
        dd("Erreur: Apikey non defini");
        if (empty($this->site_id))
        dd("Erreur: Site_id non defini");
        ...
        if($action == "payment")
        {
          ...
          dd("Erreur: Amount non defini");
          if (empty($param['currency']))
          dd("Erreur: Currency non defini");
          if (empty($param['customer_name']))
          dd("Erreur: Customer_name non defini");
          if (empty($param['description']))
          dd("Erreur: description non defini");
          if (empty($param['customer_surname']))
          dd("Erreur: Customer_surname non defini");
          if (empty($param['notify_url']))
          dd("Erreur: notify_url non defini");
          if (empty($param['return_url']))
          dd("Erreur: return_url non defini");
        }
        elseif ($action == "paymentCard")
        {
          if (empty($param['customer_email']))
          dd("Erreur: customer_email non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_phone_number']))
          dd("Erreur: custom_phone_number non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_address']))
          dd("Erreur: Customer_address non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_city']))
          dd("Erreur: customer_city non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_country']))
          dd("Erreur: customer_country non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_state']))
          dd("Erreur: Customer_address non defini (champs requis pour le paiement par carte)");
          if (empty($param['customer_zip_code']))
          dd("Erreur: customer_zip_code non defini (champs requis pour le paiement par carte)");
        }
      ...
      private function callCinetpayWsMethod($params, $url, $method = 'POST')
      {
          ...
          if (function_exists('curl_version')) {
              ...
              dd("Vous devez activer curl ou allow_url_fopen pour utiliser CinetPay");
          }
      }
      //getData
